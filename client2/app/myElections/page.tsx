  ...
  return (
    <div className="flex flex-col w-full h-screen dark:bg-[#2c2c2c]" >
      ...
      <div className="flex w-full items-center justify-center items-center  dark:bg-[#2c2c2c]" >
        <NavBar2 className="absolute z-30 mt-[24px]" navMenuOpen={navMenuOpen} setNavMenuOpen={setNavMenuOpen} button2Img={"/notification.png"} button2ClassName='hidden' />
      </div>
        <div className=" flex w-full h-[70%] items-center justify-center dark:bg-[#2c2c2c]" >
            <div className="flex flex-col items-center mt-[10%] w-[80%] h-[100%] " >
                <ElectionsSearchBar setParticipatedFilter={setParticipatedfilter} setStatusFilter={setStatusFilter} status={statusfilter} participated={participatedFilter}/>
                {/* Hardcoding for now. will map later the contents from backend service */}
                
                <div className="grid grid-cols-3  w-full h-full gap-[10px] content-start " >
                  {
                    ElectionsData
                    .map( (election)=> <ElectionCard key={election.id} title={election.title} description={election.description} pinned={election.pinned} status={election.status} participated={election.participated} bookmarked={election.bookmarked} /> )
                  }
