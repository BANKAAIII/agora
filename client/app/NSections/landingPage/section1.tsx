...
import NavBar from '@/app/NComponents/NavBar'
import BorderButton from '@/app/NComponents/borderButton'
import NoBorderButton from '@/app/NComponents/noBorderButton'
import {easeIn, easeOut, motion} from "framer-motion";
import { useState } from 'react';
import { AnimatePresence } from 'framer-motion';
import Image from 'next/image';
import Introduction from '../walletSections/introduction';
import { useRouter } from 'next/navigation';
import Rainbow from '../walletSections/rainbow'
              ...
              className="fixed top-0 left-0 w-full h-screen bg-[#666666] z-50 shadow-xl">
                        ...
                        <div className={`flex flex-row items-center justify-between w-full h-[80px] mt-[24px]  pr-[25px] md:pr-[50px] `}>
                          ...
                          <button className="w-[40px] h-[40px] sm:h-[40px] sm:w-[40px] md:w-[50px] md:h-[50px] xl:h-[74px] xl:w-[74px]  bg-[#F4F4F4] rounded-[15px]  flex items-center justify-center cursor-pointer" onClick={()=>setOpen(!open)} >
                            <Image width={36} height={36} src={"/wallet.png"} alt={""} className="h-[25px] w-[25px] md:h-[36px] md:w-[36px] " />
                          </button>
                        ...
                        <div className="w-full h-[811px] grid grid-cols-[1fr_4fr]" >
                          ...
                          <div className=' flex flex-col w-full h-full items-center justify-center ' >
                            ...
                            { walletMiniPage === "coinBase" && <div> CoinBase Wallet Connection Mini Page </div> }
                            { walletMiniPage === "metaMask" && <div> MetaMask Wallet Connection Mini Page </div> }
                            { walletMiniPage === "walletConnect" && <div> WalletConnect Wallet Connection Mini Page </div> }
                          </div>
                        </div>
